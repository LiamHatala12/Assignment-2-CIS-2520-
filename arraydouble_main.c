...
processFasta(char *filename, double *timeTaken)
{
	...
	do {
		...
		* we know something is happening */
		...
		if (status == 0) {
			...
			// if recordnumber is greater than or equal to data size
			if (recordNumber >= DataSize){

				// if data size is 0 you cant multiply it so set it to 1
				if (DataSize == 0){ 
					DataSize = 1; 
				...
				array = realloc(array, (DataSize)*sizeof(FASTArecord));

			} 
				array[recordNumber] = fRecord; 
				recordNumber++;
	...
	wastePercentage = (((double)(DataSize - recordNumber)) / DataSize) * 100; 

	printf(" %d FASTA records -- %d allocated (%.3lf%% wastePercentage)\n", recordNumber, DataSize, wastePercentage); 
	...
	free(array); //freeing memory of the array
