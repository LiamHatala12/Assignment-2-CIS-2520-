...
#include <time.h>
#include <errno.h>
#include "LLvNode.h"//include costom file
...
int processFasta(char *filename, double *timeTaken)
{
    ...
    do {
        ...
        if (status == 0) {
            ...
            PNode = llAppend(PNode, CNode);

            lineNumber += status;
            recordNumber++;

            // Optionally print the FASTA record
	...
	llFree(PNode, NULL, NULL); //free list using function in LLvNode
